# -*- coding: utf-8 -*-
"""Copy of [Klasifikasi]_Submission_Akhir_BMLP_Dimas_Eka_Maulana.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1hwp8i76uWJ8tF_gzPNK7eNYtmmd4RCrg
...
"""**Analisis Pembagian Data (Data Splitting):**
* **Metode yang digunakan:** `train_test_split` dengan rasio 80:20, `stratify=y`, dan `random_state=42`.
* **Alasan penggunaan:** Proporsi 80:20 memberikan jumlah data latih yang cukup bagi model untuk mempelajari pola tanpa kekurangan data uji. Parameter `stratify=y` memastikan distribusi kelas `Target` pada data latih dan uji tetap seimbang, sedangkan `random_state=42` menjaga hasil pembagian data tetap konsisten setiap kali kode dijalankan.
* **Hasil yang didapat:** Dataset berjumlah 1.945 data berhasil terbagi secara seimbang menjadi 1.556 data latih (80%) dan 389 data uji (20%).
...
"""**Analisis Pemodelan Decision Tree:**
* **Metode yang digunakan:** Decision Tree Classifier (`DecisionTreeClassifier`) dengan `random_state=42`.
* **Alasan penggunaan:** Memenuhi kriteria wajib (*mandatory*) *submission*. Decision Tree memetakan keputusan secara hierarkis, sangat intuitif untuk diinterpretasikan, serta efisien dalam proses pelatihan sebagai model dasar (*baseline*).
* **Hasil yang didapat:** Model *baseline* berhasil dilatih menggunakan data latih dan tersimpan secara otomatis ke dalam berkas `decision_tree_model.h5`.
...
"""**Analisis Eksplorasi Model (Random Forest):**
* **Metode yang digunakan:** Random Forest Classifier (`RandomForestClassifier`) dengan `random_state=42`.
* **Alasan penggunaan:** Memenuhi kriteria *Skilled*. Sebagai algoritma *ensemble*, Random Forest menggabungkan banyak *decision tree* untuk mengurangi risiko *overfitting* serta meningkatkan stabilitas prediksi.
* **Hasil yang didapat:** Model berhasil dilatih dengan hasil evaluasi sempurna (akurasi 1.00) pada data uji, serta tersimpan ke dalam berkas `explore_RandomForest_classification.h5`.
"""
...
* **Metode yang digunakan:** Grid Search Cross-Validation (`GridSearchCV`) dengan 5-fold cross-validation.
* **Alasan penggunaan:** Memenuhi kriteria *Advanced*. Teknik ini secara sistematis menguji kombinasi hyperparameter (`n_estimators`, `max_depth`, `min_samples_split`) untuk mencari konfigurasi model paling optimal.
* **Hasil yang didapat:** Diperoleh model hasil *tuning* yang tetap mempertahankan performa maksimal (akurasi 1.00) pada data uji, dan tersimpan ke dalam berkas `tuning_classification.h5`.
